...
function makeConfigStoreStub(group: Group = GROUP) {
  return {
    ...
    listGroups: (): Group[] => [group],
    getGlobalSettings: (): GlobalSettings => GLOBAL_SETTINGS,
  };
}

function logEntry(line: string): LogEntry {
  return { ts: 1, stream: 'stdout', level: null, line };
}
...
describe('ProcessManager.clearLogs', () => {
  it('really empties the buffer (not just the view) and keeps the reference', () => {
    ...
    pm.pushLog(id, logEntry('a'));
    pm.pushLog(id, logEntry('b'));
...
describe('ProcessManager.listLogBuffers', () => {
  it('lists every retained buffer with its id and line count', () => {
    const pm = new ProcessManager(makeConfigStoreStub());
    pm.pushLog(makeCommandId('g1', 'c1'), logEntry('a'));
    pm.pushLog(makeCommandId('g1', 'c1'), logEntry('b'));
    pm.pushLog(makeActionId('g1', 'a1'), logEntry('x'));
...
describe('ProcessManager.resolveTarget — all 4 pid kinds', () => {
  ...
  it('resolves prescript pid for second script in step', () => {
    ...
    if (!r) throw new Error('Expected resolved target');
    expect(r.kind).toBe('prescript');
    if (r.kind !== 'prescript') throw new Error('Expected prescript target');
...
describe('parseProcessId — all 4 pid kinds resolve correctly', () => {
  it('cmd: → kind command', () => {
    ...
    if (p.kind !== 'command') throw new Error('Expected command pid');
  ...
  it('act: → kind action', () => {
    ...
    if (p.kind !== 'action') throw new Error('Expected action pid');
    expect(p.groupId).toBe('g1');
